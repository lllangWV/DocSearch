...
def main():
    ...
    store = VectorStore(
        index_store_dir=index_store_dir, embed_model=embed_model, llm=llm
    )

    if DOWNLOAD_DATA:
        print("Downloading minutes")
        parse_and_download_wvu_minutes(
            download_dir=raw_dir, url="https://bog.wvu.edu/minutes"
        )

    if PROCESS_PDFS:
        print("Processing pdfs")
        processor = PDFProcessor(model=llm, max_tokens=max_tokens)
        ...
        for pdf_file in pdf_files:
            ...
            pdf_output_dir = os.path.join(interim_dir, pdf_name)

            # Use export_full to process and save both JSON and images
            pdf_data, image_paths = processor.export_full(
                pdf_path=pdf_file,
